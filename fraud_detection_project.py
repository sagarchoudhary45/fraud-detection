...
if 'transaction_date' in data.columns:
    ...
    data['transaction_date'] = data['transaction_date'].view('int64') / 10**9  # Convert to seconds since epoch
...
# Evaluate models
def evaluate_model(preds, y_test, model, X_test):
...
rf_metrics = evaluate_model(rf_preds, y_test, rf, X_test)
svm_metrics = evaluate_model(svm_preds, y_test, svm, X_test)
nn_metrics = evaluate_model(nn_preds, y_test, nn, X_test)
